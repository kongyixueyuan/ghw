...
type Ghw_Wallet struct {
	Ghw_PrivateKey ecdsa.PrivateKey
	Ghw_PublicKey  []byte
}

// 重成新的钱包
func Ghw_NewWallet() *Ghw_Wallet {
	private, public := rwq_newKeyPair()
	wallet := Ghw_Wallet{private, public}
	return &wallet
}

// 获取钱包地址
func (w Ghw_Wallet) Ghw_GetAddress() []byte {
	pubKeyHash := Ghw_HashPubKey(w.Ghw_PublicKey)
...
func Ghw_HashPubKey(pubKey []byte) []byte {
...
// 取前四位跟 解码后的地址后四位对比
func Ghw_ValidateAddress(address string) bool {
