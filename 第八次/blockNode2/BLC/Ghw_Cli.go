...
func (cli Ghw_CLI) Ghw_Run()  {
	...
	if createWalletCmd.Parsed() {
		cli.ghw_createWallet(nodeID)
	}

	if listAddressesCmd.Parsed() {
		cli.ghw_listAddrsss(nodeID)
	}
	...
	if createBlockchainCmd.Parsed() {
		...
		cli.ghw_createblockchain(*createBlockchainAddress,nodeID)
	}

	if printChainCmd.Parsed() {
		cli.ghw_printchain(nodeID)
	}
	...
	if sendCmd.Parsed() {
		...
		if strings.Contains(*sendFrom,"[") && strings.Contains(*sendFrom,"]") {
			...
			for index, fromAdress := range from {
				if !Ghw_ValidateAddress(fromAdress) || !Ghw_ValidateAddress(to[index]) {
					fmt.Println("地址无效。。")
					os.Exit(1)
				}
			}
			amount := JSONToArray(*sendAmount)
			cli.ghw_send(from, to, amount, nodeID, *sendMine)
		}else{
	...
	if getBalanceCmd.Parsed() {
		...
		cli.ghw_getBalance(*getBalanceAddress,nodeID)
	}
	if reindexUTXOCmd.Parsed() {
		cli.ghw_reindexUTXO(nodeID)
	}

	if printUTXOCmd.Parsed(){
		cli.ghw_printutxo(nodeID)
	}
	if getBalanceAllCmd.Parsed(){
		cli.ghw_getBalanceAll(nodeID)
	}
	if startNodeCmd.Parsed(){
		...
		cli.ghw_startNode(nodeID, *startNodeMiner)
	}
